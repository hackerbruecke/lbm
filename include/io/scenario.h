...
template<typename lattice_model>
std::unique_ptr<Domain < lattice_model>>

parse_scenario_file(const std::string& filename, Config& cfg, FluidCollision <lattice_model>& collision)
{
  ...
  std::cout << "Reading scenario: \"" << scenario_name << "\"..." <<
            std::endl;
  ...
  if (vtk_file) {
    ...
    throw std::logic_error("Neither vtk-file nor xl/yl/zl attribute provided to domain node!");
  }
...
// Iterate through boundary conditions
  for (auto xml_boundary = xml_domain.child("boundary"); xml_boundary; xml_boundary = xml_boundary.next_sibling(
    "boundary")) {
